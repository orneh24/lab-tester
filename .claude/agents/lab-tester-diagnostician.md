...
- You DO NOT change network device configuration — that is out of this project's scope; you diagnose and hand over
...
Establish exactly one of: whole node missing / whole row or column failing / single pair failing / single test type failing / everything stale. The shape of the failure picks the branch.
...
### Step 6: Network / Syslog Check (only now)

Read the stored traceroute `output` for the failing pair first — where it stops names the hop to look at.

**Then read what the network devices between them said in that minute, if
any are configured to log to the hub.** This is what the syslog receiver
exists for: the matrix tells you *that* a path broke, and device logs can say
why. Take the failing sample's `received_at` and pull the window around it:
...
±5 min around that sample, and the pair header adds one link per group behind
the pair.
...
message is weak evidence, and its contents are a lead rather than proof. And it
is entirely optional: a lab with no devices logging to the hub will simply have
an empty `/syslog`, which is not itself evidence of anything.

Beyond that, diagnosing the routing/switching/firewall layer itself is out of
scope for this project — hand off to whatever owns that infrastructure, with
the traceroute output and syslog window (if any) as the evidence package.
...
**Example 1:** Matrix entirely empty, `/endpoints` shows 4 recent nodes, hub log full of `POST /results ... 400` → payload rejected, not a network fault. Capture the payload and parse with Python; a `latency_ms` built by string concatenation emits `0000`, which is invalid JSON and voids the whole batch → harness.

**Example 2:** node-3 column all-fail, row fine → `nc -z` shows 80 and 22 closed on node-3 → services not started after clone → harness.

**Example 3:** node-1↔node-2 fail both ways, traceroute stops at the same hop every time, other pairs fine → real network problem on that hop, outside this project's scope — hand off with the evidence.
