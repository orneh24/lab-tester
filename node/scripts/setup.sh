#!/bin/sh
# setup.sh — First-boot setup for a mesh-probe node.
...
if [ -f "$CONFIG_FILE" ]; then
    ...
    _gi_hub=$(read_guestinfo "meshprobe.hub_url")
    _gi_group=$(read_guestinfo "meshprobe.group")
    _gi_subnet=$(read_guestinfo "meshprobe.subnet")
    _gi_host=$(read_guestinfo "meshprobe.hostname")
    ...
    _dns_server=$(read_guestinfo "meshprobe.dns_server")
    _dns_server="${_dns_server:-${DNS_SERVER:-}}"
    _dns_query=$(read_guestinfo "meshprobe.dns_query")
    ...
    # `|| _var=""` guards each read: firstboot only checks guestinfo.meshprobe.hub_url
...
_gi_host_now=$(read_guestinfo "meshprobe.hostname")
...
#
# Strip any previous mesh-probe block first so re-running setup.sh does not
# accumulate duplicate entries.
TMP_CRON="/tmp/mesh-probe-crontab.$$"
crontab -l 2>/dev/null \
    | grep -v '^# mesh-probe' \
    | grep -v 'mesh-probe/register.sh' \
    | grep -v 'mesh-probe/test-cycle.sh' \
    > "$TMP_CRON" || true

cat >> "$TMP_CRON" <<'EOF'
# mesh-probe cron entries
*/5 * * * * /usr/local/bin/mesh-probe/register.sh >> /var/log/mesh-probe/register.log 2>&1
* * * * * /usr/local/bin/mesh-probe/test-cycle.sh >> /var/log/mesh-probe/test-cycle.log 2>&1
EOF
...
if [ -d "${SRC_DIR}/../services" ]; then
    ...
    cp -f "${SRC_DIR}/../services/mesh-probe-httpd.conf" /etc/httpd.conf 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/iperf3.initd" /etc/init.d/iperf3 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/httpd.initd"  /etc/init.d/mesh-probe-httpd 2>/dev/null || true
    mkdir -p /etc/samba 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/smb.conf"   /etc/samba/smb.conf 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/smbd.initd" /etc/init.d/mesh-probe-smbd 2>/dev/null || true
    mkdir -p /etc/smtpd 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/smtpd.conf"  /etc/smtpd/smtpd.conf 2>/dev/null || true
    cp -f "${SRC_DIR}/../services/smtpd.initd" /etc/init.d/mesh-probe-smtpd 2>/dev/null || true
    chmod +x /etc/init.d/iperf3 /etc/init.d/mesh-probe-httpd /etc/init.d/mesh-probe-smbd /etc/init.d/mesh-probe-smtpd 2>/dev/null || true
...
<html>
<head>
  <title>${MY_HOSTNAME} - mesh-probe</title>
  <style>
...
# fail after the first restart. mesh-probe-httpd is a real service, enabled at boot.
rc-update add mesh-probe-httpd default 2>/dev/null || true
rc-service mesh-probe-httpd restart 2>/dev/null || rc-service mesh-probe-httpd start 2>/dev/null || \
    log "WARNING: could not start mesh-probe-httpd"
...
# Start mesh-probe-smbd (SMB probe server) if enabled
if [ "${ENABLE_SMB:-false}" = "true" ]; then
    rc-update add mesh-probe-smbd default 2>/dev/null || true
    rc-service mesh-probe-smbd start 2>/dev/null || true
    log "mesh-probe-smbd (SMB probe server) started"
else
    log "SMB disabled (ENABLE_SMB=${ENABLE_SMB:-false})"
fi

# Start mesh-probe-smtpd (SMTP probe server) if enabled
if [ "${ENABLE_SMTP:-false}" = "true" ]; then
    rc-update add mesh-probe-smtpd default 2>/dev/null || true
    rc-service mesh-probe-smtpd start 2>/dev/null || true
    log "mesh-probe-smtpd (SMTP probe server) started"
else
