# /etc/profile.d/lab-tester-status.sh — show the last test cycle plus a
# usage hint at interactive login. Same guard as
# lab-tester-node-setup.sh (login-setup.sh): interactive shells with a real
...
case "$-" in
    *i*)
        if [ -t 0 ] && [ -f /etc/lab-tester/.setup-done ]; then
            ...
            /usr/local/bin/test-status 2>/dev/null || cat /run/lab-tester/last-cycle.txt 2>/dev/null
            echo
