...
description: The lab-tester hub contract — Flask endpoints, SQLite schema, result JSON shape, and dashboard data flow. Read before changing hub/app/app.py, adding an API route, altering the results schema, or touching anything a test VM posts to.
origin: lab-tester
---

# Lab-Tester Hub API

The hub (`hub/app/app.py`) is the single source of truth for endpoints and results. Test VMs are dumb clients: they register, pull the mesh list, test, and push. Any change to the shapes below is a breaking change for every deployed golden image.
...
## Contract (do not break without reflashing test VMs)

`POST /register` — body must contain all four:

```json
{"hostname": "vm-r1", "ip": "10.1.1.50", "subnet": "10.1.1.0/24", "router": "R1"}
```

Upsert keyed on `hostname`; `last_seen` is set server-side to UTC ISO-8601. Missing any field → 400.

`GET /endpoints` — array of `{hostname, ip, subnet, router, last_seen}`, ordered by router then hostname. Test VMs skip their own hostname when iterating.

`POST /results`:

```json
{"source": "vm-r1", "results": [{
  "target_hostname": "vm-r2", "target_ip": "10.2.1.50",
...
- Identity is the parsed hostname with the source IP as fallback; there is no IP→router map. A router whose syslog hostname differs from its `ROUTER_NAME` label will not match the dashboard's filtered link — the unfiltered ±5 min link is the reliable one.
...
no access list and no `set-ntp-clients`. Point routers and VMs at real upstream
time. The app exposes the hub's own state:
...
- `endpoints.hostname` is PRIMARY KEY — hostnames must be unique per clone. A duplicated hostname silently overwrites another VM's registration; that is the most common cause of a "missing" VM in the matrix.
...
- The client's own `timestamp` field stays ISO-8601 and is passed through untouched — it is a record, never a filter. Filter on `received_at`, which the hub controls and which is immune to test-VM clock drift.
...
- Set `PRAGMA busy_timeout` on any new connection. The syslog receiver is a second writer, the SNMP poller a third; without it a result submission during a burst fails with `database is locked` and the test VM loses that whole cycle.
- SNMP polling (`hub/app/snmp_poller.py`, `snmp_client.py`) is opt-in (`HUB_SNMP_ENABLED`) and must source every packet from `HUB_MGMT_IP` — the routers' SNMP ACL only answers that address. That's why it's a hand-rolled stdlib SNMPv2c client rather than `pysnmp` or the net-snmp CLI tools: neither reliably exposes a local-source-bind option, and getting this wrong means silently polling from the wrong interface.
...
A dual-homed hub (NIC1 on the test-VM segment, NIC2 on a router management VLAN) is a design intent, not something the tooling builds: `set-static-ip` manages one interface and rewrites `/etc/network/interfaces` wholesale, so a second address is a hand-edit. Nothing pins `net.ipv4.ip_forward=0` either — Alpine's default is 0, but the build does not assert it. If you do dual-home it, pinning `HUB_SYSLOG_BIND` to the management address is the point of that key.
...
# Deployed test VMs post the old field names forever; results go silently empty.

# BAD: returning 200 on a malformed payload
# Test VMs only check the HTTP code; a lenient hub hides broken clients.
