...
- Set `PRAGMA busy_timeout` on any new connection. The syslog receiver is a second writer; without it a result submission during a burst fails with `database is locked` and the test VM loses that whole cycle.
